...
def transform(region: np.ndarray, mode: FilterMode, strength: int) -> np.ndarray:
    ...
    if mode == FilterMode.MOSAIC:
        result = region.copy()
        for top in range(0, height, strength):
            for left in range(0, width, strength):
                tile = region[top : top + strength, left : left + strength]
                result[top : top + strength, left : left + strength] = tile.mean(axis=(0, 1))
                cv2.rectangle(
                    result,
                    (left, top),
                    (min(width - 1, left + strength - 1), min(height - 1, top + strength - 1)),
                    (20, 20, 20),
                    1,
                )
        return result
